...
def _is_equipment_header_row(row: _Row) -> bool:
    ...
    for normalized_text in seen_norm:
        if not normalized_text.startswith("EQUIPO"):
            continue
        if _EQUIPMENT_CELL_RE.match(normalized_text) or _EQUIPMENT_HEADER_RE.match(
            normalized_text
        ):
            return True
